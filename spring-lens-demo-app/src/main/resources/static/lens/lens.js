...
const endpoints = {
    ...
    projectTools: "/internal/spring-lens/tools",
    projectToolInvocation: "/internal/spring-lens/tools/count_orders_by_status:invoke"
};
...
const translations = {
    en: {
        ...
        capabilityTool: "Runtime Tools",
        capabilityToolText: "Supports @LensTool so application code can expose callable runtime functions.",
        ...
        invokeProjectToolDesc: "List runtime tools and invoke count_orders_by_status with PAID.",
    ...
    zh: {
        ...
        capabilityTool: "运行时工具",
        capabilityToolText: "支持 @LensTool，让应用代码声明可调用的运行时函数。",
        sectionAi: "给 AI 的能力映射",
        aiTitle: "MCP 能力映射",
        slowSqlPath: "慢 SQL 路径",
        exceptionPath: "异常路径",
        probePath: "探针路径",
        projectToolPath: "运行时工具路径",
        pageAction: "页面动作：",
        aiTools: "AI 工具：",
        triggerSlowSql: "触发慢 SQL",
        triggerException: "触发异常",
        triggerProbe: "触发探针",
        invokeProjectTool: "调用运行时工具",
        ...
        invokeProjectToolDesc: "列出运行时工具，并以 PAID 作为参数调用 count_orders_by_status。",
        noAction: "尚未执行任何动作。",
        sectionResult: "结果面板",
...
function renderStaticShell() {
    ...
    mappingPs[7].innerHTML = `${t("aiTools")} <code>list_runtime_tools</code>, <code>invoke_runtime_tool</code>`;
