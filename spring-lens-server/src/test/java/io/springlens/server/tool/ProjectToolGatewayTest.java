...
class ProjectToolGatewayTest {
    ...
    @Test
    void listsProjectToolsThroughRuntimeClient() {
        ListProjectToolsTool tool = new ListProjectToolsTool(registryService, runtimeObservationClient);
    ...
    @Test
    void invokesProjectToolThroughRuntimeClient() {
        InvokeProjectToolTool tool = new InvokeProjectToolTool(registryService, runtimeObservationClient);
        ...
        assertThat(result).isEqualTo(Map.of("status", "PAID", "count", 1));
    }

    @Test
    void queriesProbeValuesThroughRuntimeClient() {
        QueryProbeValuesTool tool = new QueryProbeValuesTool(registryService, runtimeObservationClient);

        Object result = tool.execute(new ToolRequest(
                "orders-app",
                null,
                Map.of("probeId", "order.local.status", "limit", 5)
        ));

        assertThat(result).asList().singleElement().satisfies(value -> {
            ProbeValueRecord record = (ProbeValueRecord) value;
            assertThat(record.probeId()).isEqualTo("order.local.status");
        });
    }
    ...
    private static final class FakeRuntimeObservationClient implements RuntimeObservationClient {
        ...
        @Override
        public List<ProjectToolDescriptor> listProjectTools(AppRegistration registration) {
            return List.of(new ProjectToolDescriptor("count_orders_by_status", "Count by status"));
        }
        ...
        public Object invokeProjectTool(AppRegistration registration, String toolName, Map<String, Object> arguments) {
            return Map.of("status", arguments.get("status"), "count", 1);
        }
